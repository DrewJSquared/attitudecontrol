...
setTimeout(() => AttitudeEngine.initialize(AttitudeDMX, config, fixtureTypes), 500);


setTimeout(() => AttitudeEngine.startEngine(), 5000);
    ...
    for (var s = 0; s < config.scheduleBlocks.length; s++) {
    	...
    	if (thisBlock.day == currentTime.getDay() + 1) {
    		if (thisBlock.start - 1 <= currentTime.getHours() && thisBlock.start - 1 + thisBlock.height > currentTime.getHours()) {
    			currentEventBlockId = thisBlock.id;
    		}
    	}
    }
...
setInterval(function () { buildShowsPatch(); }, 5000);
